...
<script>
	...
	export default {
		...
		created(){
			...
			goods.state = goods.state == 1 ? true:false
			goods.mainPic = []
			this.form = goods
		...
		mounted(){
			...
			this.editor.create();
			this.editor.txt.html(this.form.content) // 重新设置编辑器内容
		},
		watch: {
			active(i) {
				if (i < 2) {
		...
		methods: {
			next() {
				...
				if (this.active == 3) {
					...
					this.$axios.post('http://127.0.0.1:3000/admin/goods/update',{
						_id: this.form._id, //商品id
